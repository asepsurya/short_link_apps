                            ...
                            <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                                ...
                                <tr class="hover:bg-gray-50/50 dark:hover:bg-[#1A1A22]/30 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right">
                                        @if($user->id !== auth()->id())
                                        <form action="{{ route('admin.users.destroy', $user) }}" method="POST" class="inline" id="delete-user-{{ $user->id }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" onclick="Swal.fire({
                                                    title: 'Delete user?',
                                                    text: 'WARNING: This will delete the user and all their links. This action cannot be undone!',
                                                    icon: 'error',
