<section class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <form method="post" action="{{ route('profile.token.revoke') }}" id="revoke-tokens-form">
            @csrf
            @method('delete')
            <x-danger-button type="button" onclick="Swal.fire({
                    title: 'Revoke all tokens?',
                    text: 'All your integrations will stop working immediately. This action cannot be undone.',
                    icon: 'warning',
                    ...
                    reverseButtons: true,
                    customClass: {
                        confirmButton: 'px-5 py-2.5 rounded-xl bg-red-600 text-white text-sm font-bold shadow-lg shadow-red-500/20',
                    ...
                    if (result.isConfirmed) {
                        document.getElementById('revoke-tokens-form').submit();
                    }
                })">
    ...
    <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)" class="text-sm text-gray-600 dark:text-gray-400">{{ __('All tokens revoked.') }}</p>
    @endif
</section>
